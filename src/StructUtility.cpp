...
static const bool TryConvertToText(CommandLineKeyValue *cmd, KeyboardStringPrintAction *actionOut)
{

  if (cmd->keyLength == 2 && cmd->key[0] == 'K' && cmd->key[1] == 'W')
  {
...
static const bool TryConvertToMidi(CommandLineKeyValue *cmd, PressionRequest *actionOut, MidiAction *midiOut)
{
  ...
  else if (startIndex > -1 && startIndex == endIndex)
  { // TWO VALUE
...
{
  if (cmd->keyLength == 2 && cmd->key[0] == 'K' && cmd->key[1] == 'C')
  {
